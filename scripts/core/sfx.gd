...
func _synth(name: String) -> AudioStreamWAV:
	...
	for i in n:
		...
		match name:
			...
			"pop":
				# v17 wooden-ball knock (owner: "exactly like wooden balls
				# rolling on wooden tables - like 8 Ball Pool"): a warm
				# low-mid BODY (310/465 Hz) with a soft woody knock at
				# ~980 Hz and only a whisper of attack noise - long enough
				# decay to read as solid wood, never a plastic click.
				var body := (sin(TAU * 310.0 * t) * 0.55 + sin(TAU * 465.0 * t) * 0.32) * exp(-38.0 * t)
				var knock := sin(TAU * 980.0 * t) * 0.16 * exp(-130.0 * t)
				var click := (randf() * 2.0 - 1.0) * 0.10 * exp(-650.0 * t)
				v = body + knock + click
			"block":
